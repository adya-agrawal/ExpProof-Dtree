...
def lime_fidelity(model, X_test, scaler, K=50, n_neigh=300, mode="gaussian"):
    ...
    for idx in indices:
        ...
        # 1) Sample neighbors
        Z, w = sample_neighborhood(x, n_neigh, d, scaler, mode=mode)
        ...
        nn_lbls_Z = (nn_prob >= 0.5).astype(int)        # NN labels on sampled neighborhood
        sur_lbls_Z = (reg.predict(Z) >= 0.5).astype(int)  # LIME surrogate labels
...
# ---------------------------------------------------------
# DECISION TREE Fidelity (tree is PASSED IN already trained)
# ---------------------------------------------------------
